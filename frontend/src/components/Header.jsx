...
const Header = ({ setShowLogin }) => {
	...
	return (
		<header
			className={`${
				header ? '!py-3 bg-secondary shadow-sm' : ''
		...
		>
			...
			<div className='flexBetween gap-x-20'>
				{/* Navbar Dekstop */}
				<Navbar
					containerStyles={'hidden md:flex gap-x-5 lx:gap-x-10 medium-15'}
				/>
				{/* Navbar Mobile */}
				<Navbar
					containerStyles={`${
						menuOpened
							...
							: 'flex items-start flex-col gap-y-12 fixed top-20 p-12 bg-white rounded-3xl w-64 shadow-md medium-16 ring-1 ring-slate-900/5 transition-all duration-300 -right-[100%]'
					}`}
				/>
			</div>
